#' @title Exact t-Test Regression with Instrumental Variables
#' @description Performs an exact t-test regression with instrumental variables, with optional optimization using a genetic algorithm.
...
#' @param Z.var The name of the instrumental variable(s).
...
exacttIV <- function(formula, 
  ...
  #X.var <- X.var[!X.var %in% Z.var]
  Z.var <- Z.var[!Z.var %in% X.var]
  ...
  XFormulaString <- paste("~", paste(X.var, collapse = " + "))
  X <- stats::model.matrix(stats::as.formula(XFormulaString), data)
  assign <- attr(X, "assign")
  
  ZFormulaString <- paste("~", paste(c(Z.var, 0), collapse = " + "))
  Z <- model.matrix(stats::as.formula(ZFormulaString), data)
  ...
  # 2SLS
  ivregObject <- ivreg::ivreg(formula, data = data)
  ...
  for(i in seq_along(assign)){
    ...
    if(optimize){
      ...
      results[["gaResults"]][[colnames(X)[i]]] <- gaResults
    }
    ...
    results[["detailed"]][[colnames(X)[i]]] <- detailed.out[[i]]
  ...
  results[["summary"]] <- summaryTable.out
