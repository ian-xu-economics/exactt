...
Citing the `exactt` package is convenient. The text version and BibTeX entry is provided by:
```{r}
citation("exactt")
```
...
The main function in `exactt` is `exactt()`. It computes the $(1-\alpha)\%$ confidence interval for coefficients in a linear model, with much flexibility. To see all that `exactt()` provides, it is best to run it on some examples. 
...
If optimization is desired, we can set `optimize` to `TRUE`. We can also pass through whatever other parameters that the `GA::ga()` function normally accepts. For instance, we can set `maxiter = 10`, `parallel = TRUE`, and `seed = 2024`. Note that if optimize is `FALSE` and the other parameters are still passed through, optimization will _not_ be performed.
